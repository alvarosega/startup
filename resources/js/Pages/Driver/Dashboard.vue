            ...
            <div class="flex-1 overflow-y-auto p-4">
                ...
                <div v-if="activeOrder" class="space-y-4 pb-10">
                    ...
                    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                        <div v-if="activeOrder.status === 'preparing'" class="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
                            <p class="text-sm font-black text-gray-800">Recoge el paquete</p>
                            <p class="text-xs text-gray-500 mt-1">El despachador debe marcar la orden como "Despachada" en su panel para que puedas continuar.</p>
                        </div>

                        <button v-else-if="activeOrder.status === 'dispatched'" 
                                @click="markAsArrived(activeOrder.id)" 
                                :disabled="isLoading" 
                                class="w-full bg-blue-600 text-white text-sm font-black py-4 rounded-xl shadow-lg hover:bg-blue-700 uppercase tracking-widest disabled:opacity-50 transition-all">
