...
import { 
    ...
    ExternalLink, AlertTriangle, Building2,
    Cpu, Mail, Phone, XCircle, Info, Camera
...
// Accedemos a .data porque viene de un JsonResource
...
const form = useForm({
    ...
    // RECTIFICACIÓN: El estado debe ser uno de: pending, approved, suspended
    status: d.status, 
        ...
        <div class="px-4 py-6 max-w-7xl mx-auto space-y-6">
            ...
            <div v-if="Object.keys(form.errors).length > 0" class="p-4 bg-destructive/10 border-l-4 border-destructive animate-in fade-in slide-in-from-top-4">
                <div class="flex items-center gap-3 text-destructive mb-2">
                    ...
                    <span class="font-mono font-black text-xs uppercase tracking-widest">Error de Integridad de Datos</span>
                </div>
            ...
            <div class="flex justify-between items-center border-b border-primary/30 pb-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-mono text-muted-foreground flex items-center gap-2 uppercase">
                            <Cpu :size="12" /> STATUS: 
                            <span :class="{
                                'text-amber-500': form.status === 'pending',
                                'text-emerald-500': form.status === 'approved',
                                'text-destructive': form.status === 'suspended'
                            }" class="font-black italic underline">{{ form.status }}</span>
                        </p>
                    </div>
                </div>

                <button @click="submit" :disabled="form.processing" 
                        class="px-6 py-3 bg-primary text-primary-foreground font-mono text-xs border border-primary/50 hover:shadow-neon-primary transition-all flex items-center gap-2 italic font-black disabled:opacity-50">
                    <Save :size="16" /> {{ form.processing ? 'SINCRONIZANDO...' : 'GUARDAR_CAMBIOS' }}
                </button>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div v-show="activeTab === 'info'" class="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500">
                    ...
                    <div class="space-y-4">
                        <div class="border border-border p-6 bg-card/30 space-y-6 relative overflow-hidden">
                            ...
                            <div class="space-y-5">
                                <div class="space-y-2">
                                    <label class="text-[10px] font-mono text-muted-foreground block uppercase font-bold">Sucursal Asignada *</label>
                                    <select v-model="form.branch_id" class="w-full bg-background border border-border p-3 font-mono text-xs focus:border-primary outline-none">
                                ...
                                <div class="space-y-2">
                                    <label class="text-[10px] font-mono text-muted-foreground block uppercase font-bold">Estado en Sistema *</label>
                                    <select v-model="form.status" class="w-full bg-background border border-border p-3 font-mono text-xs focus:border-primary outline-none">
                                ...
                                <div v-if="form.status === 'suspended'" class="space-y-2 animate-in slide-in-from-top-2">
                                    <label class="text-[10px] font-mono text-destructive block uppercase font-bold tracking-widest">Motivo de Suspensión / Rechazo</label>
                                    <textarea v-model="form.rejection_reason" placeholder="EXPLIQUE EL MOTIVO AL CONDUCTOR..." 
                                              class="w-full bg-background border border-destructive/30 p-3 font-mono text-xs outline-none h-24 focus:border-destructive uppercase" />
                                </div>
                            </div>
                        </div>

                        <div class="border border-border p-6 bg-muted/20 space-y-3">
                            <h4 class="text-[9px] font-mono font-black text-muted-foreground uppercase tracking-[0.2em]">Canales de Comunicación</h4>
                    ...
                    <div class="lg:col-span-2 border border-border p-8 bg-card/20 space-y-8">
                        <h3 class="font-mono font-black text-xs text-primary border-b border-primary/20 pb-4 uppercase">Información de Unidad e Identidad</h3>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-2">
                                <label class="text-[10px] font-mono font-bold text-muted-foreground uppercase italic">// Nombres</label>
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-mono font-bold text-muted-foreground uppercase italic">// Apellidos</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                            <div class="space-y-2">
                                <label class="text-[10px] font-mono font-bold text-muted-foreground uppercase italic">// Vehículo</label>
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-mono font-bold text-muted-foreground uppercase italic">// Nº Licencia</label>
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-mono font-bold text-muted-foreground uppercase italic">// Placa</label>
                ...
                <div v-show="activeTab === 'documents'" class="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-500">
                    <div v-for="doc in [
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <a v-if="doc.path" :href="doc.path" target="_blank" class="text-cyan-500 hover:text-cyan-400 transition-colors flex items-center gap-2">
                                <span class="text-[8px] font-black uppercase tracking-tighter">Ver Original</span> <ExternalLink :size="14" />
                            </a>
                        </div>
                        ...
                        <div class="aspect-video bg-black/40 border-2 border-dashed border-border flex items-center justify-center overflow-hidden relative">
                            ...
                            <div v-else class="text-center space-y-2">
                                ...
                                <span class="text-[8px] font-mono text-muted-foreground uppercase tracking-widest">Documento_Faltante</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2 p-6 bg-primary/5 border border-dashed border-primary/30 flex gap-4 items-start">
                        ...
                        <div class="space-y-1">
                            <p class="text-[10px] font-mono font-black text-primary uppercase tracking-widest">Verificación de Identidad y Base</p>
                            <p class="text-[9px] font-mono text-muted-foreground leading-relaxed uppercase">
                                Al pasar a "APROBADO", el conductor podrá loguearse y aceptar órdenes de la sucursal asignada. 
                                Si existen dudas sobre la legitimidad de los documentos, mantenga el estado en "PENDIENTE" y contacte al sujeto.
                            </p>
                        </div>
                    </div>
                </div>
