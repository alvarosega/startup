...
import { 
    ...
    Bike, Car, Camera, Eye, EyeOff, Copy, ChevronRight, Info // <--- INFO IMPORTADO
...
// DESEMPAQUETADO SEGURO DEL RESOURCE (Regla 3.C)
const d = props.driver.data || props.driver;
...
const driverCode = computed(() => {
    // Si tienes UUID, mostramos un hash corto
    const shortId = String(d.id).includes('-') ? d.id.split('-')[0] : String(d.id).padStart(4, '0');
    return `DRV_${shortId.toUpperCase()}`;
});
...
const vehicleTypeLabel = (type) => {
    const types = { moto: 'MOTOCICLETA', car: 'AUTOMÓVIL', truck: 'CAMIONETA' };
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span v-if="d.details?.license_number" class="text-[8px] font-mono border border-primary/30 px-2 py-0.5 text-primary">
                                        {{ d.details.license_number }}
                                    </span>
                                </div>
                                <div class="relative border-2 border-dashed border-primary/30 bg-background/50 aspect-video flex items-center justify-center overflow-hidden group/image">
                                    
                                    <img v-if="d.details?.license_photo_path" 
                                        :src="d.details.license_photo_path" 
                                        class="w-full h-full object-contain p-2" />
                                    ...
                                    <a v-if="d.details?.license_photo_path" 
                                    :href="d.details.license_photo_path" 
                                    target="_blank" 
                                    class="absolute inset-0 bg-background/80 opacity-0 group-hover/image:opacity-100 flex items-center justify-center transition-opacity duration-300">
                                        <span class="px-4 py-2 border border-primary text-primary text-[10px] font-mono flex items-center gap-2 bg-background">
                                            <ExternalLink :size="14" /> VER ORIGINAL
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span v-if="d.details?.license_plate" class="text-[8px] font-mono border border-primary/30 px-2 py-0.5 text-primary">
                                        {{ d.details.license_plate }}
                                    </span>
                                </div>
                                <div class="relative border-2 border-dashed border-primary/30 bg-background/50 aspect-video flex items-center justify-center overflow-hidden group/image">
                                    
                                    <img v-if="d.details?.vehicle_photo_path" 
                                        :src="d.details.vehicle_photo_path" 
                                        class="w-full h-full object-contain p-2" />
                                    ...
                                    <a v-if="d.details?.vehicle_photo_path" 
                                    :href="d.details.vehicle_photo_path" 
                                    target="_blank" 
                                    class="absolute inset-0 bg-background/80 opacity-0 group-hover/image:opacity-100 flex items-center justify-center transition-opacity duration-300">
                                        <span class="px-4 py-2 border border-primary text-primary text-[10px] font-mono flex items-center gap-2 bg-background">
                                            <ExternalLink :size="14" /> VER ORIGINAL
