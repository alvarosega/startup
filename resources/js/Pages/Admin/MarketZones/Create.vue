...
const submit = () => {
    form.post(route('admin.market-zones.store'), {
        preserveScroll: true,
        onError: () => {
...
<template>
    <AdminLayout>
        <Head title="Alta de Zona Logística" />
        ...
        <div class="max-w-4xl mx-auto pb-12 px-4 md:px-0">
            ...
            <div class="mb-8 relative group/header">
                ...
                <div class="relative z-10 flex justify-between items-end">
                    <div>
                        ...
                        <h1 class="text-3xl font-display font-black tracking-widest text-primary uppercase glitch-text drop-shadow-[0_0_12px_hsl(var(--primary)/0.6)] leading-none" data-text="NUEVA ZONA">
                            NUEVA ZONA
                        </h1>
                        <p class="text-[10px] font-mono text-muted-foreground mt-1 flex items-center gap-2">
                            <Cpu :size="12" class="text-primary animate-pulse" /> CONFIGURACIÓN DE ÁREA LOGÍSTICA
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="p-6 bg-background/80 backdrop-blur-sm border border-primary/30 flex justify-end items-center relative">
                    <button type="submit" :disabled="form.processing" class="px-8 py-3 bg-primary text-primary-foreground text-[10px] font-mono font-black uppercase shadow-neon-primary hover:bg-primary/90 transition-all relative group/submit overflow-hidden">
                        ...
                        <span v-else class="flex items-center gap-2 relative z-10"><Save :size="14" /> REGISTRAR ZONA</span>
            ...
            <div class="mt-4 text-center">
                <p class="text-[8px] font-mono text-muted-foreground">
                    SESSION_ID // ZON_CREATE_INIT // {{ new Date().toISOString().slice(0,10) }}
                </p>
            </div>
        </div>
    </AdminLayout>
