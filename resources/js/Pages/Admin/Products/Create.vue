...
// 3. LÓGICA DE VALIDACIÓN (Nombre Único)
...
// LEY: Validación unificada (Solo un nivel de categoría)
...
const tempCode = computed(() => `PRD_NEW_${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`);
</script>
            ...
            <form @submit.prevent="submit">
                <div id="product-card" 
                    ...
                    <div class="p-6 md:p-8 space-y-6 relative z-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2 space-y-1.5">
                                <label class="text-[10px] font-mono font-bold text-primary uppercase tracking-widest mb-2 flex items-center gap-1">
                                ...
                                <div class="relative group/input">
                                    ...
                                    <input v-model="form.name" 
                                           type="text" 
                                           class="w-full pl-10 pr-12 py-3 bg-background border border-border/50 font-mono text-lg font-bold focus:border-primary focus:shadow-neon-primary outline-none transition-all uppercase"
                                           :class="{'border-destructive/50': form.errors.name || !isNameAvailable, 'border-cyan-500/50': isNameAvailable && form.name}"
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select v-model="form.brand_id" 
                                        class="w-full px-4 py-3 bg-background border border-border/50 font-mono text-sm focus:border-primary focus:shadow-neon-primary outline-none transition-all uppercase"
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-mono font-bold text-primary uppercase tracking-widest mb-2 flex items-center gap-1">
                                    <Layers :size="12" /> // CATEGORÍA <span class="text-destructive">*</span>
                                </label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="md:col-span-1">
                                ...
                                <div @click="$refs.imgInput.click()"
                                     class="aspect-square border-2 border-dashed border-primary/30 bg-primary/5 flex items-center justify-center cursor-pointer overflow-hidden relative group/image hover:border-primary transition-all">
                                    
                                    <img v-if="masterImagePreview" :src="masterImagePreview" class="w-full h-full object-cover transition-transform duration-500 group-hover/image:scale-110" />
                                    
                                    <div v-else class="flex flex-col items-center gap-2">
                                        <UploadCloud :size="32" class="text-primary/50 group-hover/image:text-primary transition-colors" />
                                        <span class="text-[8px] font-mono text-primary/50 uppercase tracking-widest">SUBIR_IMAGEN</span>
                                    </div>
                            ...
                            <div class="md:col-span-2 space-y-4">
                                <div class="space-y-1.5">
                                    ...
                                    <textarea v-model="form.description" 
                                              rows="4" 
                                              class="w-full bg-background border border-border/50 px-4 py-3 font-mono text-sm focus:border-primary focus:shadow-neon-primary outline-none transition-all resize-none uppercase"
                                              placeholder="ESPECIFICACIONES TÉCNICAS DEL PRODUCTO..."></textarea>
                                </div>
                                ...
                                <div class="flex gap-4">
                                    <div @click="form.is_alcoholic = !form.is_alcoholic"
                                         ...
                                         :class="form.is_alcoholic ? 'border-warning/30 bg-warning/5' : 'border-border/50 hover:border-primary/30'">
                                        <div class="flex items-center justify-between">
                                            <span class="text-[10px] font-mono font-bold uppercase flex items-center gap-2" :class="form.is_alcoholic ? 'text-warning' : 'text-muted-foreground'">
                                                <Wine :size="14" /> ALCOHOL
                                            </span>
                                            <div class="relative w-10 h-5 border border-border/50">
                                                <div class="absolute top-0.5 left-0.5 w-4 h-4 transition-all duration-300" :class="form.is_alcoholic ? 'translate-x-5 bg-warning shadow-[0_0_8px_hsl(var(--warning))]' : 'translate-x-0 bg-muted-foreground'"></div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div @click="form.is_active = !form.is_active"
                                         class="flex-1 border p-3 cursor-pointer transition-all relative group/toggle"
                                         :class="form.is_active ? 'border-cyan-500/30 bg-cyan-500/5' : 'border-border/50 hover:border-primary/30'">
                                        <div class="flex items-center justify-between">
                                            <span class="text-[10px] font-mono font-bold uppercase flex items-center gap-2" :class="form.is_active ? 'text-cyan-500' : 'text-muted-foreground'">
                                                <component :is="form.is_active ? Wifi : WifiOff" :size="14" /> {{ form.is_active ? 'ACTIVO' : 'INACTIVO' }}
                                            </span>
                                            <div class="relative w-10 h-5 border border-border/50">
                                                <div class="absolute top-0.5 left-0.5 w-4 h-4 transition-all duration-300" :class="form.is_active ? 'translate-x-5 bg-cyan-500 shadow-[0_0_8px_hsl(var(--primary))]' : 'translate-x-0 bg-muted-foreground'"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="p-6 bg-background/80 backdrop-blur-sm border-t border-primary/30 flex justify-between items-center">
                        <Link :href="route('admin.products.index')" class="px-6 py-2 border border-destructive/50 text-destructive font-mono text-xs hover:bg-destructive hover:text-destructive-foreground transition-all relative group/cancel">
                            CANCELAR
                        </Link>

                        <button type="submit" :disabled="form.processing || !isValidToContinue"
