...
// --- LOGICA SKU (UX MEJORADA) ---
const addSku = async () => {
    // Insertar al inicio (unshift)
    form.skus.unshift({ 
        name: form.name ? `${form.name} ` : '', 
        code: '', price: 0, conversion_factor: 1, weight: 0, image: null
    });
    
    // Sincronizar refs visuales al inicio
...
const removeSku = (index) => {
    if (form.skus.length <= 1) return alert("Se requiere al menos una variante.");
...
const copyNameToSku = () => {
    // Solo si es el primer SKU y está vacío
...
const validateStep = () => {
    ...
    if (currentStep.value === 1) {
        if (!form.name.trim()) return form.setError('name', 'Nombre obligatorio') && false;
        if (!form.brand_id) return form.setError('brand_id', 'Marca obligatoria') && false;
        if (!form.category_id) return form.setError('category_id', 'Categoría obligatoria') && false;
        copyNameToSku();
    }
    return true;
};
...
const submit = () => {
    form.clearErrors();
    // Validación básica de duplicados EAN
...
<template>
    <AdminLayout>
        ...
        <div class="max-w-5xl mx-auto pb-24 md:pb-12">
            ...
            <div class="mb-6 px-4 md:px-0">
                <div class="flex justify-between items-center mb-4">
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground mt-0.5">Configuración Maestra y Variantes</p>
                    </div>
                ...
                <div class="relative px-2">
                    ...
                    <div class="absolute top-1/2 left-0 h-1 bg-primary -z-10 -translate-y-1/2 rounded-full transition-all duration-500 ease-out" 
                    ...
                    <div class="flex justify-between">
                        <div v-for="step in steps" :key="step.id" 
                            ...
                            <div class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center border-2 transition-all bg-card shadow-sm"
                                 :class="currentStep === step.id ? 'border-primary text-primary scale-110 shadow-md ring-4 ring-primary/10' : 
                                         currentStep > step.id ? 'border-success bg-success text-white' : 'border-border text-muted-foreground'">
                                <CheckCircle v-if="currentStep > step.id" :size="16" />
                                <component v-else :is="step.icon" :size="16" />
                            </div>
                            <span class="text-[9px] md:text-[10px] font-bold uppercase bg-background px-1"
            ...
            <div v-if="Object.keys(form.errors).length > 0" class="mx-4 md:mx-0 mb-4 alert alert-error py-2 px-3 text-xs">
                    ...
                    <div class="p-4 md:p-8 flex-1 relative">
                        <Transition name="fade" mode="out-in">
                            ...
                            <div v-if="currentStep === 1" key="1" class="space-y-5 animate-in slide-in-from-right-4 fade-in">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                    <div class="md:col-span-2 space-y-1.5">
                                        <label class="form-label text-sm">Nombre Maestro *</label>
                            ...
                            <div v-else-if="currentStep === 2" key="2" class="space-y-5 animate-in slide-in-from-right-4 fade-in">
                                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    <div class="lg:col-span-1 space-y-2">
                                        <label class="form-label text-sm">Imagen Maestra</label>
                                        <input ref="masterImageInputRef" type="file" class="hidden" accept="image/*" @change="e => handleImage(e, 'master')" />
                                        
                                        <div class="relative w-full aspect-square border-2 border-dashed border-border bg-muted/10 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-primary transition overflow-hidden group"
                                    ...
                                    <div class="lg:col-span-2 space-y-4">
                                        <div class="space-y-1.5">
                                            ...
                                            <textarea v-model="form.description" rows="3" class="form-input resize-none" placeholder="Características..."></textarea>
                                        </div>
                                        
                                        <div class="bg-card border border-border rounded-xl divide-y divide-border">
                                            <div class="p-3 border-b border-border flex items-center justify-between cursor-pointer" @click="form.is_active = !form.is_active">
                                            ...
                                            <div class="p-3 flex items-center justify-between cursor-pointer" @click="form.is_alcoholic = !form.is_alcoholic">
                            ...
                            <div v-else key="3" class="space-y-4 animate-in slide-in-from-right-4 fade-in">
                                    ...
                                    <div v-for="(sku, index) in form.skus" :key="index" 
                                        ...
                                        <div class="flex gap-3">
                                            <div class="shrink-0 w-20">
                                                ...
                                                <div class="w-20 h-20 rounded-lg border border-dashed border-border bg-muted/5 flex items-center justify-center cursor-pointer overflow-hidden relative group/img"
                                                    ...
                                                    <div v-if="skuImagePreviews[index]" class="absolute inset-0 bg-black/30 hidden group-hover/img:flex items-center justify-center">
                                                        <UploadCloud :size="12" class="text-white"/>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex-1 grid grid-cols-2 gap-3">
                                                <div class="col-span-2">
                                                    ...
                                                    <input v-model="sku.name" type="text" class="form-input w-full h-8 text-xs font-bold" 
                                                ...
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <input v-model="sku.code" type="text" class="form-input w-full h-8 text-xs pl-6 font-mono" 
                                                               placeholder="770..." 
                                                ...
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <input v-model="sku.conversion_factor" type="number" step="1" min="1" 
                                                               class="form-input w-full h-8 text-xs pl-6 text-center" 
                                                ...
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <input v-model="sku.price" type="number" step="0.01" 
                                                               class="form-input w-full h-8 text-xs pl-6 text-success font-bold" 
                                                ...
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <input v-model="sku.weight" type="number" step="0.01" 
                                                               class="form-input w-full h-8 text-xs pl-6" 
                                                               :class="{'border-error': form.errors[`skus.${index}.weight`]}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div v-if="form.errors.skus" class="alert alert-error text-xs p-2 text-center">
                                    {{ form.errors.skus }}
                                </div>
                            </div>
