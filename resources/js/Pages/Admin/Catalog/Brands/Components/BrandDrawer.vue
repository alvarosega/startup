...
watch(
    // Clausura de protección para evitar evaluar propiedades de options si llega undefined
    () => [props.show, props.brand, props.options],
    ([isOpen, currentBrand, currentOptions]) => {
        if (!isOpen || !currentOptions) return;
        form.clearErrors();

        const providersList = currentOptions.providers || [];
        const categoriesList = currentOptions.categories || [];
        ...
        if (currentBrand) {
            ...
            // Resolución polinómica blindada contra nulos e hidrataciones corruptas
            form.provider_id = currentBrand.provider_id || currentBrand.provider?.id || providersList.find(p => p.name === currentBrand.provider_name)?.id || '';
            form.category_id = currentBrand.category_id || currentBrand.category?.id || categoriesList.find(c => c.name === currentBrand.category_name)?.id || '';
            ...
            // Forzar preselección mitigando envíos nulos involuntarios evaluando la existencia del índice
            form.provider_id = providersList[0]?.id || '';
            form.category_id = categoriesList[0]?.id || '';
        }
    },
    { immediate: true }
);
...
const filteredParents = computed(() => {
    const parentsList = props.options?.parents || [];
    if (!props.brand) return parentsList;
    return parentsList.filter(parent => parent.id !== props.brand.id);
});
...
<template>
    <div v-if="show" class="fixed inset-0 z-50 overflow-hidden" role="dialog" aria-modal="true">
        ...
        <div class="absolute inset-y-0 right-0 max-w-full flex pl-10">
            <div class="w-screen max-w-xl bg-white dark:bg-neutral-900 border-l border-neutral-200 dark:border-neutral-800 shadow-2xl flex flex-col h-full text-neutral-900 dark:text-neutral-50 font-mono text-xs">
                
                <div class="px-5 py-4 border-b border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 flex items-center justify-between select-none font-sans">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-neutral-400 dark:text-neutral-500 uppercase tracking-wider mt-0.5">Configuración y enlace atómico de la marca</p>
                        </div>
                    </div>
                ...
                <form @submit.prevent="submit" class="flex-1 overflow-y-auto p-5 space-y-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <span v-if="form.errors.name" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.name }}</span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input v-model="form.slug" type="text" @input="isManualSlug = true" placeholder="Conversión automática si se omite" class="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-md px-3 py-2 text-xs text-neutral-900 dark:text-neutral-50 focus:border-neutral-400 dark:focus:border-neutral-700 focus:ring-0 outline-none transition-colors" />
                            <span v-if="form.errors.slug" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.slug }}</span>
                        </div>
                        ...
                        <div class="space-y-1.5 select-none">
                            ...
                            <div class="flex items-center gap-2 border border-neutral-200 dark:border-neutral-800 rounded-md px-2 bg-neutral-50 dark:bg-neutral-950 h-[34px]">
                                ...
                                <span class="text-xs font-bold uppercase text-neutral-900 dark:text-neutral-50">{{ form.bg_color }}</span>
                            </div>
                            <span v-if="form.errors.bg_color" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.bg_color }}</span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-neutral-50 dark:bg-neutral-950 p-3 rounded-md border border-neutral-200 dark:border-neutral-800 select-none">
                        <div class="space-y-1.5">
                            ...
                            <select v-model="form.provider_id" required class="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-md px-3 py-2 text-xs font-bold uppercase text-neutral-900 dark:text-neutral-50 focus:border-neutral-400 dark:focus:border-neutral-700 focus:ring-0 outline-none transition-colors">
                                <option v-for="prov in (options?.providers || [])" :key="prov.id" :value="prov.id">
                                    {{ (prov.name || 'PROVEEDOR_SIN_NOMBRE_COMERCIAL').toUpperCase() }}
                                </option>
                            </select>
                            <span v-if="form.errors.provider_id" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.provider_id }}</span>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <span v-if="form.errors.category_id" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.category_id }}</span>
                        </div>
                        ...
                        <div class="md:col-span-2 border-t border-neutral-200 dark:border-neutral-800 pt-2.5 mt-1 space-y-1.5">
                            ...
                            <span v-if="form.errors.parent_id" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.parent_id }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <span v-if="form.errors.website" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.website }}</span>
                        </div>
                        ...
                        <div class="space-y-1.5 select-none">
                            ...
                            <div class="border border-dashed border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950 rounded-md p-3 flex items-center gap-3 relative min-h-[64px]">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-[9px] text-neutral-400 dark:text-neutral-500">Formatos: PNG, WEBP, SVG</p>
                                </div>
                            </div>
                            <span v-if="form.errors.image" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.image }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea v-model="form.description" rows="3" class="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-md px-3 py-2 text-xs uppercase text-neutral-900 dark:text-neutral-50 focus:border-neutral-400 dark:focus:border-neutral-700 focus:ring-0 outline-none transition-colors定位 resize-none" placeholder="Información comercial del fabricante..."></textarea>
                        <span v-if="form.errors.description" class="text-[10px] text-rose-600 dark:text-rose-400 block mt-0.5">{{ form.errors.description }}</span>
                    </div>
                </form>

                <div class="px-5 py-3 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 flex items-center justify-end gap-2 select-none shrink-0 font-sans">
                    ...
                    <button type="button" @click="submit" :disabled="form.processing" class="bg-neutral-900 hover:bg-neutral-800 dark:bg-white dark:hover:bg-neutral-200 text-white dark:text-neutral-950 px-4 py-1.5 border border-transparent rounded-md transition-colors text-xs font-bold uppercase tracking-wider inline-flex items-center gap-1.5 disabled:opacity-50">
