        ...
        <div class="w-full min-h-screen bg-background pb-32">
            ...
            <div class="px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16 mt-12">
                <div class="relative group">
                    <div class="aspect-square rounded-[3.5rem] overflow-hidden bg-muted/20 border border-border/50 flex items-center justify-center p-12 relative">
                        <img 
                            :src="bundle.data.image_url" 
                            class="w-full h-full object-contain transition-transform group-hover:scale-105 z-10" 
                            @error="(e) => e.target.src = '/assets/img/bundle_banner_noeditable.webp'"
                        />
                        <div class="absolute inset-0 bg-primary/10 blur-[50px] rounded-full scale-75 group-hover:scale-90 transition-transform duration-700"></div>
                    </div>
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <div class="space-y-4 mb-16">
                        <div v-for="item in bundle.data.items" :key="item.id" class="flex items-center gap-5 p-4 bg-card border border-border/50 rounded-3xl">
                            <div class="w-14 h-14 bg-muted/30 rounded-2xl p-2 flex-shrink-0">
                                <img :src="item.image" class="w-full h-full object-contain" @error="(e) => e.target.src = '/assets/img/sku_placeholder.png'"/>
                            </div>
                            <div class="flex-1">
                    ...
                    <div class="bg-card border-2 border-primary/20 rounded-[3rem] p-10 flex items-center justify-between shadow-2xl">
                        <div class="flex flex-col">
                            ...
                            <span class="text-5xl font-black tracking-tighter italic">Bs. {{ Number(bundle.data.fixed_price).toFixed(2) }}</span>
                        </div>
