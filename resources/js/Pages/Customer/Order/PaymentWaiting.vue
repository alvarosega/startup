...
const manualRefresh = () => {
    ...
    router.reload({
        only: ['order'], // QUERY LAW: Solo pedimos el objeto de la orden actualizado
    ...
    <ShopLayout>
        ...
        <div class="max-w-2xl mx-auto p-6 min-h-[90vh] pb-32">
            ...
            <div class="product-card glass-titanium !rounded-3xl border border-border/10 overflow-hidden relative shadow-f1-glow">
                ...
                <div class="h-1.5 w-full bg-blue-500/20">
                    <div class="h-full bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)] animate-pulse-fast w-full"></div>
                </div>

                <div class="p-8 md:p-12 text-center">
                    
                    <div class="relative w-28 h-28 mx-auto mb-8">
                        <div class="absolute inset-0 bg-blue-500/10 rounded-3xl blur-2xl animate-pulse"></div>
                        <div class="relative w-full h-full bg-background/40 border border-blue-500/20 rounded-3xl flex items-center justify-center shadow-inner">
                            <Clock :size="48" class="text-blue-500 animate-spin-slow" />
                        </div>
                    </div>
...
.product-card {
    animation: reveal 0.8s cubic-bezier(0.22, 1, 0.36, 1) backwards;
}
