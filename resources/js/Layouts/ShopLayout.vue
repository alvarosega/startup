    ...
    <div class="bg-background text-foreground font-sans flex flex-col selection:bg-primary selection:text-primary-foreground transition-colors duration-300 relative"
        ...
        <nav class="absolute top-0 left-0 right-0 h-[10svh] flex items-center bg-background/40 backdrop-blur-xl border-b border-white/5"
            ...
            <div class="container mx-auto px-4 h-full relative flex items-center justify-between gap-4 w-full">
                ...
                <div v-if="!isSearchOpen" class="flex-1 max-w-md hidden md:block">
                ...
                <div class="flex items-center gap-1 sm:gap-3 shrink-0">
                    <button v-if="!isSearchOpen" @click="toggleSearch" class="p-2 hover:bg-muted rounded-full transition-colors"><Search :size="20" /></button>

                    <Link :href="route('customer.cart.index')" class="relative p-2 hover:bg-muted rounded-full transition-colors">
                        <ShoppingBag :size="20" />
                    ...
                    <button @click="showAccountMenu = !showAccountMenu" class="w-9 h-9 rounded-xl overflow-hidden border-2 transition-colors" :class="showAccountMenu ? 'border-primary' : 'border-white/10 hover:border-white/30'">
                        <img v-if="user && user.avatar" :src="`/assets/avatars/${user.avatar}`" class="w-full h-full object-cover">
                        <div v-else class="w-full h-full bg-muted flex items-center justify-center text-muted-foreground"><User :size="18" /></div>
                    </button>
                    ...
                    <Transition name="menu-pop">
                        <div v-if="showAccountMenu" class="absolute right-4 top-[11svh] w-64 bg-card/95 backdrop-blur-2xl border border-white/10 rounded-[24px] shadow-2xl overflow-hidden z-[70]">
                            <div v-if="user" class="p-5 bg-primary/5 border-b border-white/5">
                                ...
                                <p class="font-bold truncate text-sm text-foreground">{{ user.name || user.email }}</p>
                            </div>
                            <nav class="p-2">
                                <template v-if="user">
                                    <Link v-for="item in menuItems" :key="item.route" :href="route(item.route)"
                                        @click="showAccountMenu = false"
                                        class="flex items-center gap-3 px-4 py-3 text-xs font-bold text-foreground hover:bg-primary/10 rounded-xl transition-all">
                                        <component :is="item.icon" :size="16" /> {{ item.name }}
                                    </Link>
                                    <div class="h-px bg-border/50 my-2"></div>
                                    <button @click="logout" class="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-error hover:bg-error/10 rounded-xl transition-all">
                                        <LogOut :size="16" /> Cerrar Sesión
                                    </button>
                                </template>
                                <template v-else>
                                    <div class="p-2 grid grid-cols-2 gap-2">
                                        <Link :href="route('login')" class="btn btn-primary btn-sm rounded-xl">Login</Link>
                                        <Link :href="route('register')" class="btn btn-outline btn-sm rounded-xl border-border">Unirse</Link>
                                    </div>
                                </template>
                            </nav>
                        </div>
        ...
        <main class="w-full mx-auto transition-all pt-[10svh] flex flex-col flex-1" 
            ...
            <div v-if="isProfileSection && user" class="flex flex-col md:flex-row gap-8 flex-1">
                <aside class="hidden md:block w-64 shrink-0 bg-card rounded-2xl border border-border p-4 h-fit">
                    <h3 class="text-xs font-black uppercase tracking-wider text-muted-foreground mb-4 px-2">Mi Cuenta</h3>
                    <nav class="flex flex-col gap-1">
                        <Link v-for="item in menuItems" :key="item.route" :href="route(item.route)"
                            class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl transition-all"
                            :class="route().current(item.route) ? 'bg-primary text-primary-foreground shadow-md' : 'text-foreground hover:bg-muted'">
        ...
        <footer class="h-[5svh] border-t border-border bg-muted/20 flex items-center shrink-0">
        ...
        <div v-if="showAccountMenu || showAccessibilityMenu" class="fixed inset-0 z-[55] bg-background/20 backdrop-blur-sm" @click="showAccountMenu = false; showAccessibilityMenu = false"></div>
    </div>
