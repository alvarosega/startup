    ...
    public function execute(PurchaseDataDTO $dto, string $adminId): Purchase
    {
        return DB::transaction(function () use ($dto, $adminId) {
            ...
            foreach ($dto->items as $item) {
                ...
                $generatedLotCode = $item->lotCode ?? "LOT-{$prefix}-{$currentYear}-{$sequence}";
                ...
                InventoryMovement::insert([
                    ...
                    'reference' => "Compra Interna: {$dto->documentNumber}",
                    'reason' => 'Ingreso automatizado por orden de compra',
                    'created_at' => now(),
                ]);
