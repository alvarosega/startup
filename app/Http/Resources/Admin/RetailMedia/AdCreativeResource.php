...
class AdCreativeResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'campaign_name'      => $this->relationLoaded('campaign') ? mb_strtoupper((string) $this->campaign->name) : null,
            'placement_id'       => (string) $this->placement_id,
            'placement_name'     => $this->relationLoaded('placement') ? mb_strtoupper((string) $this->placement->name) : null,
            ...
            'category_name'      => $this->relationLoaded('category') ? mb_strtoupper((string) $this->category->name) : null,
            'brand_id'           => (string) $this->brand_id,
            'brand_name'         => $this->relationLoaded('brand') ? mb_strtoupper((string) $this->brand->name) : null,
            'bundle_id'          => (string) $this->bundle_id,
            'bundle_name'        => $this->relationLoaded('bundle') ? mb_strtoupper((string) $this->bundle->name) : null,
